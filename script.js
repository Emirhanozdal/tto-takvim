...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- ADMİN DURUMUNU KONTROL ETME VE ARAYÜZÜ GÜNCELLEME ---
    const checkAdminStatus = () => {
        const user = auth.currentUser();
        const wasAdmin = isAdmin; // Önceki durumu sakla
    ...
    const updateUserPanel = (user) => {
        ...
        if (user) {
            // Giriş yapılmışsa: E-posta ve Çıkış Yap butonu
            userPanel.innerHTML = `<span>${user.email}</span><button id="logout-btn">Çıkış Yap</button>`;
            document.getElementById('logout-btn').addEventListener('click', () => {
                user.logout().then(() => {
                    // Çıkış yapıldığında durumu anında güncelle
                    checkAdminStatus();
                });
            });
        } else {
            // Giriş yapılmamışsa: Yönetim Paneli linki
    ...
    if (document.getElementById('entry-list')) {
