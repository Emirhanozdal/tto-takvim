document.addEventListener('DOMContentLoaded', () => {
    ...
    if (window.netlifyIdentity) {
        ...
        netlifyIdentity.on('login', user => {
            enableAdminMode();
            netlifyIdentity.close(); // Giriş sonrası widget'ı kapat
        });
    ...
    const renderYearView = () => {
        ...
        for (let i = 0; i < 12; i++) {
            ...
            monthBox.innerHTML = `<h3>${monthNames[i]}</h3>`;
            const monthKey = `${currentYear}-${String(i + 1).padStart(2, '0')}`;
            if (entries[monthKey] && entries[monthKey].some(e => e.type === 'project')) {
                monthBox.classList.add('has-project');
            }
            monthBox.addEventListener('click', () => {
                currentDate = new Date(currentYear, i, 1);
                showMonthView();
            });
            yearViewBody.appendChild(monthBox);
        }
    }
    
    // --- AY GÖRÜNÜMÜ FONKSİYONLARI (SABİT 42 GÜN) ---
    const renderCalendar = () => {
        ...
        for (let i = 0; i < 42; i++) {
            ...
            if (isProjectMonth && currentDay.getMonth() === month) {
            ...
            if (entries[dateKey] && entries[dateKey].length > 0) {
                ...
                entries[dateKey].filter(e => e.type === 'event').forEach(event => {
    ...
    entryForm.addEventListener('submit', (e) => {
        ...
        if (monthView.classList.contains('hidden')) renderYearView();
        else renderCalendar();
